...
//     // First n elements (smallest) b minimum sum
//     for (int i = 0; i < n; i++) {
//         cout << arr[i] << " ";
//         minSum += arr[i];
//     }

//     // Next n elements (largest) b maximum sum
...
//  The expression n & (n b 1) essentially turns off the least significant set bit in n.
...
//     for(int i=0;i<k;i++){
//         sum+=max(0,happiness[i]-i);//used max to eleminate -ve entries from distorting answer
//     }
//     cout<<sum;
// }
...
//         for(int j=0;j<n;j++){
//             sum+=abs(matrix[i][j]);
//             countNegvalu+= matrix[i][j]<0 ? 1 : 0; //add 1 if -ve value, else add 0
...
//     int n=nums.size();
//     int first=nums[0];//1st element will definitely be a valid min cost
//     int second=INT_MAX;
//     int third=INT_MAX;
//     for(int i=1;i<n;i++){
//         if(nums[i]<second){
...
//     int n=nums.size();
//     for(int i=0;i<n;i++){
//         int shift= nums[i]%n; // %n used to calculate how many steps to move
//         int newindex=nums[i]<0 ? (i-abs(shift) +n)%n : (i+shift)%n; // %n is used in circular array/strings 
...
//     int uniqueCombo=pow(2,k);
//     for(int i=k;i<=n;i++){
//         string a=s.substr(i-k,k);
//         if(!st.count(a)){
//             st.insert(a);
//             uniqueCombo--;
//             if(uniqueCombo==0) return true;
//         }
//         else{
//             return false;
//         }
...
//         }
//         if(k<ceil(len/2.0)){
//             return findKthBit(n-1,k);//as if k<len/2 then it can be calculate from kth digit of n-1
//         }else if(k==ceil(len/2.0)){
//             return '1';
//         }else{
//             char ch = findKthBit(n-1, len-(k-1)); //handled reversed by taking length - (k-1) element of n-1
//             return (ch == '0') ? '1' : '0'; //handled flipped bit by doing 1 if 0, else kep 0
//         }
//     }
// };
...
//     int count=0,j=0;
//     for(int i=0;i<n;i++){
//         j^=1;//perform xor operation each time
//         if(s[i]-'0' == j){
//             count++;
//         }
//     }
//     cout<<min(count,n-count);//count is when string like 1010... and n-count is when 0101..
// }
...
//             count++;
//             i++;//if condition sattisfied then increement i (that is increemnt double ..sort of obv..)
//             //else move with normal i in for loop to prevent out of bound
//         }
//     }
//     cout<<count;
// }
...
// int main(){
//     string s = "1001";
//     // if 01 appears (even one) then it is always not a contigous set of ones
//     int n=s.length();
//     for(int i=1;i<n;i++){
//         if(s[i-1]-'0'==0 && s[i]-'0'== 1) cout<<"false";
//     }
// }
...
// int main(){
//     string s = "11000111";
//     int ones=0,zeros=0,curr0=0,curr1=0;
//     int n=s.length();
//     for(int i=0;i<n;i++){
//         if(s[i]=='1'){
//             curr1++;
//             curr0=0;//if there is 1 then continouity of 0 broken
//         }
//         if(s[i]=='0'){
//             curr0++;
//             curr1=0;//if there is 0 then continouity of 1 broken
//         }
//         ones=max(ones,curr1);//to count max continous Segment of 1
//         zeros=max(curr0,zeros);//to count max continous Segment of 0
//     }
//     if(ones>zeros) cout<<"true";
// }
...
// int main(){
//     string s = "101011";
//     // if 00 or 11 appears then add 1 to result and skip next index
//     int n=s.length();
//     int res=0;
//     for(int i=1;i<n;i++){
//         if((s[i-1]-'0'==1 && s[i]-'0'== 1) || (s[i-1]-'0'==0 && s[i]-'0'== 0)){
//             res++;
//             i++;
//         }
//     }
...
//Use binary search on answer, whenever you see 'minimizing the maximum'
...
//         for(int j=0;j<n;j++){
//             if(matrix[i][j]==1 && i>0){
//                 matrix[i][j]+=matrix[i-1][j]; //to find cummulative continous sum
//             }
//         }
//         vector<int> heights=matrix[i];//vector to store heights of each column
//         sort(heights.begin(),heights.end(),greater<int>());
//         for(int i=0;i<n;i++){
//             int base=i+1;//formula for base
...
//     vector<int> col(m);
//     for(int i=0;i<n;i++){
//         int row=0;//reset row=0 after i updates
//         for(int j=0;j<m;j++){
//             col[j]+=grid[i][j];//to take sum of jth column
//             row+=col[j];
//             if(row <= k){
//                 res++;
//             }
...
//     int n=mat.size();//as square matrix
//     bool rot90=true,rot180=true,rot270=true,rot0=true;
//     for(int i=0;i<n;i++){
//         for(int j=0;j<n;j++){
...
//         }
//     }
//     if(rot0 || rot90 || rot180 || rot270) cout<<"true";
// }
...
//     //vectors to calculate prefix and suffix of i'th elements and store
//     prefix[0]=1,suffix[n-1]=1;
//     for(int i=1;i<n;i++){
//         prefix[i]=prefix[i-1]*nums[i-1];//to calculate prefix product
//     }
//     for(int i=n-2;i>=0;i--){
//         suffix[i]=suffix[i+1]*nums[i+1];//to calculate suffix product
//     }
//     for(int i=0;i<n;i++){
//         nums[i]=prefix[i]*suffix[i];//to calculate result
//     }
//     for(int c:nums) cout<<" "<<c;
// }

// Another approach for 238
// #include <bits/stdc++.h>
// using namespace std;
// int main(){
//     vector<int> nums = {1,2,3,4};
...
//     for(int i=0;i<n;i++){
//         result[i] *= prefix;
//         prefix *= nums[i];//to calculate prefix product
//     }
//     for(int i=n-1;i>=0;i--){
//         result[i] *= suffix;
//         suffix *= nums[i];//to calculate suffix product
//     }
...
//     //only use one extra grid instead of prefix and suffix seprate, save space
    
//     //Used same prefix /suffix approach as in 238
//     long long suffix=1;
//     for(int i = n-1; i >= 0; i--){
//         for(int j = m-1; j >= 0; j--){
//             result[i][j] = suffix;
...
// using namespace std;
// int main(){
//     vector<vector<int>> grid ={{1,3},{4,2}};
//     int n=grid.size();
//     int m=grid[0].size();
//     int sum=0;
//     for(int i=0;i<n;i++){
//         for(int j=0;j<m;j++){
//             sum+=grid[i][j];//00,01,10,11
//         }
//     }
//     // cout<<sum;
...
//     int curr=0;
//     int target=sum/2;
//     for(int i=0;i<n;i++){// to check horizontal cuts
//         for(int j=0;j<m;j++){
//             curr +=grid[i][j]; // 00,10,01,11
//         }
//         if(curr == target){
//             cout<<true;
//         }
//     }
//     curr=0;//intialize curr to 0 again for 
//     for(int i=0;i<m;i++){ //to check vertical cuts
//         for(int j=0;j<n;j++){
//             curr +=grid[j][i];
//         }
//         if(curr == target){
...
//                 if(mat[i][j]!=mat[i][(j + k)%n]) cout<<false; //% n to handle overflow
//             }else{//if odd row
//                 if(mat[i][j]!=mat[i][(j - k%n + n )%n]) cout<<false; 
//             }
//         }
//     }
// }
...
//     if(s1[0]!=s2[0] && s1[2]!=s2[2]) swap(s1[0],s1[2]);
//     if(s1[1]!=s2[1] && s1[3]!=s2[3]) swap(s1[1],s1[3]);
//     if(s1==s2){
//         cout<<true;
//     }else cout<<false;
// }
...
//     if(num==0) hexD="0";
//     while(n!=0){
//         int rem=n%16;//hexadecimal is 16 bit so used 16
//         if(rem>=10){
//             hexD+=(rem-10+'a');//97 is a (-10 as the remainder is 10 ahead)
//         }else{
//             hexD+=(rem+'0');//48 is 0
//         }
//         n=n/16;//reduce num;
